...
--Image format
In this project, the size of all images needs to be 256 x 256. You can run the following code to resize all the images in a folder:		python transfer.py --folder_path Folder_path --size 256                        
(Folder_path is the path to the folder where you store your images).
...
3. Training & Test
--VGGnet & ResNet
Training VGG model:		Python VGGTrain.py
...
Training ResNet model:		Python ResnetTrain.py
...
Test model of /TrainedModel/VGGnet.pth:		Python VGGTest.py

Test model of /TrainedModel/Resnet.pth:		Python ResnetTest.py

After testing, a confusion matrix diagram will be generated.

--Grid search
To find the best suitable combination of learning rate and batch size, grid search can be used:		python .\grid_search.py --algorithm_type 1
(_algorithm_type_ : 0 represent Resnet model and 1 represent VGGnet model.)
Best model will be saved in /Best_model.


SVM
Different from neural network model. For the SVM algorithm, just run SVM.py directly to complete train and test:		Python SVM.py

4. GUI
Running GUI.py will open the front-end interface we designed. For the operation using GUI, you can check the demo video:		Python GUI.py